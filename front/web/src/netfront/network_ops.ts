...
export const AddEdge = function (source_id: string, target_id: string) {
    ...
    if (target_node.config.type === "l2_switch") {
        ...
        let type_connection: number | null = null;
        ...
        const iface_id = l2SwitchPortUid(target_node.data.id);
        target_node.interface.push({
            id: iface_id,
            name: iface_id,
            connect: edge_id,
    ...
    if (source_node.config.type === "l2_switch") {
        ...
        const iface_id = l2SwitchPortUid(source_node.data.id);
        source_node.interface.push({
            id: iface_id,
            name: iface_id,
            connect: edge_id,
    ...
    if (target_node.config.type === "l1_hub") {
        const iface_id = l1HubPortUid(target_node.data.id);
        target_node.interface.push({
            id: iface_id,
            name: iface_id,
            connect: edge_id,
    ...
    if (source_node.config.type === "l1_hub") {
        const iface_id = l1HubPortUid(source_node.data.id);
        source_node.interface.push({
            id: iface_id,
            name: iface_id,
            connect: edge_id,
...
export const DeleteNode = function (node_id: string) {
    ...
    $.each(edges, function (idx: number, edge: any) {
        ...
        if (edge.data.source === node_id) {
            ...
            const new_iface = t.interface.filter(function (iface: any) {
...
export const DeleteEdge = function (edge_id: string) {
    ...
    const connected_nodes = [ed.data.source, ed.data.target];
    ...
    for (const node_id of iterator) {
        ...
        // Iterate interface and delete one
        const edge_node_iface = t.interface.filter(function (iface: any) {
    ...
    const getNodeLabel = function (ele: any) {
        ...
        if (n.config && n.config.type === "l2_switch") {
            ...
            if (stpMode > 0) {
                ...
                const pr =
                    n.config.priority !== undefined && n.config.priority !== null
...
export const MarkLinkDownEdges = function (cy_instance: any) {
    ...
    cy_instance
        .edges(".link-down, .link-down-active")
