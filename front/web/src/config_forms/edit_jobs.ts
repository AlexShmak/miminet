        ...
        setTimeout(() => {
            switch (job.job_id.toString()) {
                ...
                case "5": // traceroute (с опциями)
                    $("#config_host_traceroute_with_options_options_input_field").val(
                        job.arg_1 || ""
                    );
        ...
        setTimeout(() => {
            switch (job.job_id.toString()) {
                ...
                case "105": // Добавить IPIP-интерфейс
                    ...
                    $("#config_router_add_ipip_tunnel_interface_ip_input_field").val(
                        job.arg_3 || ""
                    );
                ...
                case "109": // Добавить Port Forwarding для TCP
                    ...
                    $("#config_router_add_port_forwarding_tcp_iface_select_field").val(
                        job.arg_1 || ""
                    );
                    $("#config_router_add_port_forwarding_tcp_port_input_field").val(
                        job.arg_2 || ""
                    );
                    $("#config_router_add_port_forwarding_tcp_dest_ip_input_field").val(
                        job.arg_3 || ""
                    );
                    $("#config_router_add_port_forwarding_tcp_dest_port_input_field").val(
                        job.arg_4 || ""
                    );
                    break;
                case "110": // Добавить Port Forwarding для UDP
                    ...
                    $("#config_router_add_port_forwarding_udp_iface_select_field").val(
                        job.arg_1 || ""
                    );
                    $("#config_router_add_port_forwarding_udp_port_input_field").val(
                        job.arg_2 || ""
                    );
                    $("#config_router_add_port_forwarding_udp_dest_ip_input_field").val(
                        job.arg_3 || ""
                    );
                    $("#config_router_add_port_forwarding_udp_dest_port_input_field").val(
                        job.arg_4 || ""
                    );
                    break;
                default:
